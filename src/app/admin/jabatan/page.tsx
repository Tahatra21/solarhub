...
import { Eye, Edit, Trash2, Briefcase, Calendar, Plus, Search, RefreshCcw } from 'lucide-react';
...
export default function JobPositionsPage() {
  ...
  const fetchJobPositions = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/jabatans/master?page=${currentPage}&limit=10&search=${searchQuery}`);
      ...
      if (data.success) {
        ...
        console.error('Failed to fetch job positions:', data);
        setJobPositions([]);
      }
    } catch (error) {
  ...
  const handleViewDetails = (jobPosition: JobPosition) => {
    alert(`View details for: ${jobPosition.jabatan}`);
  };

  const handleEdit = (jobPosition: JobPosition) => {
    alert(`Edit: ${jobPosition.jabatan}`);
  };

  const handleDelete = (jobPosition: JobPosition) => {
    if (confirm(`Are you sure you want to delete ${jobPosition.jabatan}?`)) {
      alert(`Delete: ${jobPosition.jabatan}`);
    }
  };

  const handleAdd = () => {
    alert('Add new job position');
  };
    ...
    <div className="p-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow border p-6 mb-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Job Positions</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Manage job positions</p>
          </div>
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {loading ? (
                ...
                jobPositions.map((jobPosition) => (
                  <tr key={jobPosition.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                          {jobPosition.created_at ? new Date(jobPosition.created_at).toLocaleDateString() : '-'}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        <Calendar className="w-4 h-4 text-gray-400" />
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                          {jobPosition.updated_at ? new Date(jobPosition.updated_at).toLocaleDateString() : '-'}
                        </span>
                      </div>
                    </td>
