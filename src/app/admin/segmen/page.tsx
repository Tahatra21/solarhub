...
import { Eye, Edit, Trash2, Layers, Calendar, Plus, Search, RefreshCcw } from 'lucide-react';
...
export default function SegmentsPage() {
  ...
  const fetchSegments = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/segmens/master?page=${currentPage}&limit=10&search=${searchQuery}`);
      ...
      if (data.success) {
        ...
        console.error('Failed to fetch segments:', data);
        setSegments([]);
      }
    } catch (error) {
  ...
  const handleViewDetails = (segment: Segment) => {
    alert(`View details for: ${segment.segmen}`);
  };

  const handleEdit = (segment: Segment) => {
    alert(`Edit: ${segment.segmen}`);
  };

  const handleDelete = (segment: Segment) => {
    if (confirm(`Are you sure you want to delete ${segment.segmen}?`)) {
      alert(`Delete: ${segment.segmen}`);
    }
  };

  const handleAdd = () => {
    alert('Add new segment');
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow border p-6 mb-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">Manage product segments</p>
          </div>
