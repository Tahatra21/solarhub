...
export async function GET() {
  try {
    ...
    const buffer = await SecureExcelService.createWorkbook(excelData, {
      ...
      filename: `Data_Produk_${new Date().toISOString().split('T')[0]}.xlsx`
    });
    
    // Return response dengan security headers
    return SecureExcelService.createExcelResponse(buffer, `Data_Produk_${new Date().toISOString().split('T')[0]}.xlsx`);
