...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `stage_${type}_${timestamp}${fileExtension}`;

    // Path direktori upload
    const uploadDir = path.join(process.cwd(), 'public', 'images', 'product', 'stage');
    ...
    const relativePath = `/images/product/stage/${fileName}`;
