...
export default function EditKategorisForm({ kategori, onSuccess, onCancel }: EditKategoriFormProps) {
  ...
  const [kategoriName, setKategoriName] = useState(kategori.nama_kategori);
  const [iconLight, setIconLight] = useState(kategori.icon_light || "");
  const [iconDark, setIconDark] = useState(kategori.icon_dark || "");
  const [iconLightPreview, setIconLightPreview] = useState(
    kategori.icon_light ? `/images/product/kategori/${kategori.icon_light}` : ""
  );
  const [iconDarkPreview, setIconDarkPreview] = useState(
    kategori.icon_dark ? `/images/product/kategori/${kategori.icon_dark}` : ""
  );
  ...
  return (
    <div className="px-6 py-4">
      <div className="border-b border-gray-200 dark:border-gray-700 pb-4 mb-6">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Edit Kategori</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui informasi kategori dan icon</p>
      </div>
